...
export default function EditEvenementForm({
  ...
  const { editSection, isPending: isEditingSection } = useEditSection();
  const { editEvent, isPending: isEditingEvent } = useEditEvent();
  ...
  const addSection = () => {
    ...
    setSections((prev) => [
      ...prev,
      {
        ...
        ...newSection,
        images: newSection.images.map((img) => ({ imgUrl: img })),
      },
    ]);
  ...
  return (
    <div className="space-y-8" ref={formRef}>
      ...
      <div className="mb-6 flex justify-end space-x-4">
        ...
        <Button onClick={handleSubmit} disabled={!isDirty || isEditingEvent}>
          {isEditingEvent ? (
            <>
