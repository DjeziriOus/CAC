    ...
    <div className="space-y-4 rounded-lg border border-primary bg-card p-6">
      ...
      <div className="space-y-2">
        ...
        <ImageUpload
          ...
          currentImage={
            images[0]?.imgUrl ? images[0]?.imgUrl : images[0] || null
          }
          onImageSelect={(image) => {
...
export default function EditEvenementForm({
  ...
  const handleDeleteSection = async (sectionId) => {
    try {
      console.log(sectionId, initialEvent.id);
      deleteSection({ id: sectionId, eventId: initialEvent.id });
  ...
  const handleSaveSection = async (updatedSection) => {
    try {
      ...
      //    id: 5,
      //    paragraph: 'Les avancées technologiques et leur rôle croissant dans la prestation des soins de santé à distance.',
      //    images: ['data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAABkAA…Q/71Tp8769ttwHyGI/h+iPWqzMBsmkgAAAABJRU5ErkJggg==']
      // }
  ...
  const handleSubmit = () => {
    ...
    try {
      ...
      toast.error("Failed to submit the form. Please try again.", {
        description: `${error.message} -- Erreur lors de la modification de l'evenement.`,
      });
    }
  };
        ...
        {isAddingSectionOpen ? (
          <div className="space-y-4 rounded-lg border border-primary bg-card p-6 shadow-sm">
            ...
            <div className="space-y-2">
              ...
              <ImageUpload
                inputId="section-image-upload"
                currentImage={newSection.images[0] || null}
                onImageSelect={(image) =>
                  setNewSection((prev) => ({ ...prev, images: [image] }))
                }
                onImageRemove={() =>
                  setNewSection((prev) => ({ ...prev, images: [] }))
                }
                height="h-48"
                loading={isUploading}
              />
